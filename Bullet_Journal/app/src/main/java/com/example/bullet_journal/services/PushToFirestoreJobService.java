...
import com.example.bullet_journal.synchronization.GetHabitDaysForSync;
import com.example.bullet_journal.synchronization.GetHabitsForSyncTask;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    AsyncTask<Void, Void, Boolean> manageHabitsTask = new GetHabitsForSyncTask(new AsyncResponse<Boolean>() {
                        @Override
                        public void taskFinished(Boolean retVal) {
                            if (retVal) {
                                Log.i("** SYNC DONE **", "HABITS, SUCCESS");
                            } else {
                                Log.i("** SYNC DONE **", "HABITS, FAILED");
                            }
                        }
                    }).execute();

                    AsyncTask<Void, Void, Boolean> manageHabitDaysTask = new GetHabitDaysForSync(new AsyncResponse<Boolean>() {
                        @Override
                        public void taskFinished(Boolean retVal) {
                            if (retVal) {
                                Log.i("** SYNC DONE **", "HABDAYS, SUCCESS");
                            } else {
                                Log.i("** SYNC DONE **", "HABDAYS, FAILED");
                            }
                        }
                    }).execute();
